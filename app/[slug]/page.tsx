...
export async function generateMetadata({ params }: {
  ...
  const { title, summary: description, category } = post
    ...
    <div className="grow md:flex space-y-8 md:space-y-0 md:space-x-8 pt-12 md:pt-16 pb-16 md:pb-20">
      ...
      <div className="grow">
        <div className="max-w-[700px]">
          ...
          <div className="mb-3">
            <Link
              className="inline-flex text-custom-yellow-500 rounded-full border border-slate-200 dark:border-slate-800 dark:bg-gradient-to-t dark:from-slate-800 dark:to-slate-800/30"
              href="/"
            >
            ...
            <header>
              <div className="flex items-center justify-between mb-1">
                ...
                <div className="text-xs text-slate-500 uppercase">
                  <span className="text-custom-yellow-500">—</span> <PostDate dateString={post.publishedAt} /> <span className="text-slate-400 dark:text-slate-600">·</span> 4 Min read
                </div>
                {/* Share buttons */}
                <ul className="inline-flex">
                  <li>
                    <a
                      className="flex justify-center items-center text-slate-400 dark:text-slate-500 hover:text-custom-yellow-500 dark:hover:text-custom-yellow-500 transition duration-150 ease-in-out"
                      href="#0"
                      aria-label="Twitter"
                    >
